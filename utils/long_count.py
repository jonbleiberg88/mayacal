...
class DistanceNumber(LongCount):
    ...
    def to_approx_years(self, pretty_print=False):
        """ Returns the approximate number of years, months and days represented by the distance number

        Note that this method approximates the length of a year as 365.25 kin
        (days) and the length of a month as 30.44 kin. It does not account for
        leap years or differing month duration.

        Args:
            pretty_print (bool): Whether to nicely format the result

        Returns:
            (str or tuple): Returns a tuple of integers in the form
                (years, months, days) if pretty_print is False. Otherwise
                returns a string in the form "x years, x months, x days".
